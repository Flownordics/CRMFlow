-- =========================================
-- CRMFlow Complete Schema Setup
-- =========================================
-- Run this entire script in your Supabase SQL Editor
-- Project: rgimekaxpmqqlqulhpgt
-- Dashboard: https://supabase.com/dashboard/project/rgimekaxpmqqlqulhpgt/sql/new
-- =========================================
...
-- User integrations table for Google Calendar integration
...
-- Events table for native calendar
create table if not exists public.events (
  ...
  company_id uuid REFERENCES public.companies(id) ON DELETE SET NULL,
  contact_id uuid REFERENCES public.people(id) ON DELETE SET NULL,
  kind text DEFAULT 'native' CHECK (kind IN ('native', 'google')),
...
CREATE POLICY "Allow authenticated users to manage user_integrations" ON public.user_integrations FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

drop policy if exists "Allow authenticated users to manage user_settings" on public.user_settings;
CREATE POLICY "Allow authenticated users to manage user_settings" ON public.user_settings FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

drop policy if exists "Allow authenticated users to manage events" on public.events;
CREATE POLICY "Allow authenticated users to manage events" ON public.events FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
...
-- ---------- Seed Data ----------
-- Pipeline + stages (one row, 5 stages in one row – without "Qualified")
...
-- stage probabilities (can be edited in Settings page)
...
-- =========================================
-- Next steps:
-- 1. Go to Authentication > Email Templates to configure email templates
-- 2. Update Netlify environment variables to use this new project
