...
drop trigger if exists trg_deal_integrations_updated_at on public.deal_integrations;
...
drop trigger if exists trg_line_items_parent_exists on public.line_items;
...
-- ---------- Seed Data ----------
-- Pipeline + stages (én række, 5 stages i én række – uden "Qualified")
...
-- stage probabilities (kan redigeres i Settings siden)
